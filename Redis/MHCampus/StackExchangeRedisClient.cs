...
namespace AWSCommon.Redis
{
    internal class StackExchangeRedisClient : IRedisClient
    {
        ...
        public bool Set<T>(string Key, T Value, DateTime? ExpiryDate = null)
        {
            ...
            var result = this.dataBase.StringSet(Key, Serializer.SerializeObject(Value));
        ...
        public IEnumerable<T> GetMultiple<T>(IEnumerable<string> Keys)
        {
            ...
            {
                var isT = Serializer.TryDeserializeObject<T>(value, out entry);
                if (!isT) continue;
                result.Add(entry);
            }
        ...
        public long AddToList<T>(string Key, IEnumerable<T> Values)
        {
            ...
            var redisValues = Values.Select(x => (RedisValue)Serializer.SerializeObject(x));
        ...
        public T GetFromList<T>(string Key, long ItemIndex)
        {
            ...
            return Serializer.DeserializeObject<T>(this.dataBase.ListGetByIndex(Key, ItemIndex));
        }
        ...
        public IEnumerable<T> GetTopXFromList<T>(string Key, int Quantity) // Quantity = 0 - ALL Items
        {
            ...
            foreach (var redisValue in redisResults)
            {
                ...
                results.Add(Serializer.DeserializeObject<T>(redisValue));
            }
        ...
        public bool RemoveFromList<T>(string Key, T ItemValue)
        {
            ...
            var redisValue = (RedisValue)Serializer.SerializeObject(ItemValue);
        ...
        public bool AddToUniqueList<T>(string Key, IEnumerable<T> Values)
        {
            ...
            var sortedSetValues = Values.Select(x => new SortedSetEntry((RedisValue)Serializer.SerializeObject(x), DateTime.Now.Ticks));
        ...
        public bool RemoveFromUniqueList<T>(string Key, IEnumerable<T> ItemsValue)
        {
            ...
            // create an actual serialized redis value and remove ALL of its instances from the currect list
            var redisValues = ItemsValue.Select(x => (RedisValue)Serializer.SerializeObject(x));
            ...
            foreach (var action in actions) {
                switch (action.ActionType)
                {
                    case eRedisTransactionActionType.Set:
                        ...
                        trans.StringSetAsync(action.Key.KeyString, Serializer.SerializeObject(entity)).ContinueWith(_ =>
                        {
                            if (hasExpiry)
                                trans.KeyExpireAsync(action.Key.KeyString, ((DateTime)action.Properties["EXPIRY"]));
                        });
                        break;
                    case eRedisTransactionActionType.AddToUniqueList:
                        var sortedSetValues = ((List<string>)action.Value).Select(x => new SortedSetEntry((RedisValue)Serializer.SerializeObject(x), DateTime.Now.Ticks));
                        trans.SortedSetAddAsync(action.Key.KeyString, sortedSetValues.ToArray());
                        break;
                    case eRedisTransactionActionType.RemoveFromUniqueList:
                        var redisValues = ((List<string>)action.Value).Select(x => (RedisValue)Serializer.SerializeObject(x));
                        trans.SortedSetRemoveAsync(action.Key.KeyString, redisValues.ToArray());
                        break;
                    case eRedisTransactionActionType.Get:
                        Task<RedisValue> taskResult = trans.StringGetAsync(action.Key.KeyString);
                        actionsResult.Add(action, taskResult);
                        break;
                    case eRedisTransactionActionType.Remove:
                        trans.KeyDeleteAsync(action.Key.KeyString);
                        break;
                }
            }
        ...
        //class encapsulates the serialization that the StackExchangeRedisClient uses so that
        //transaction actions of type 'get' can share it.
        ...
        public class Serializer
        {
            ...
            internal static bool TryDeserializeObject<T>(string Json, out T Value)
            {
                try
                {
        ...
        public class StackExchangeRedisTransactionActionWithResult<T> : RedisTransactionActionWithResult<T>
        {
            public StackExchangeRedisTransactionActionWithResult(eRedisTransactionActionType ActionType, ICacheKey Key)
                : base(ActionType, Key)
            {
                this.ValueType = typeof(T);
            }

            public override void PostProcess(object result)
            {
                Value = CastedValue = Serializer.DeserializeObject<T>(result.ToString());
            }
        }                
    }
}
