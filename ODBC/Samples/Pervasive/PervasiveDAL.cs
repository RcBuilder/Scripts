        ...
        public CompanyInfo GetCompanyInfo()
        {
            ...
            using (var connection = new OdbcConnection(this.ConnetionString))
            {
                ...
                using (var reader = command.ExecuteReader())
                {
                    ...
                    if (this.IsColumnExists($"CompDet", "NEWSLIKASAPAKNAME"))
                        config.ChargeProvider = SafeConvert.ToString(reader["NEWSLIKASAPAKNAME"]);
                    if (this.IsColumnExists($"CompDet", "NEWSLIKATERMINALNO"))
                        config.ChargeTerminal = SafeConvert.ToString(reader["NEWSLIKATERMINALNO"]);
                    if (this.IsColumnExists($"CompDet", "NEWSLIKATERMINALPASS"))
                        config.ChargePassword = SafeConvert.ToString(reader["NEWSLIKATERMINALPASS"]);
                    if (this.IsColumnExists($"CompDet", "LogoUrl1"))
                        config.LogoPath1 = SafeConvert.ToString(reader["LogoUrl1"]);
                    if (this.IsColumnExists($"CompDet", "LogoUrl2"))
                        config.LogoPath2 = SafeConvert.ToString(reader["LogoUrl2"]);
                    if (this.IsColumnExists($"CompDet", "LogoUrl3"))
                        config.LogoPath3 = SafeConvert.ToString(reader["LogoUrl3"]);
                    if (this.IsColumnExists($"CompDet", "LogoUrl4"))
                        config.LogoPath4 = SafeConvert.ToString(reader["LogoUrl4"]);
                }
            }

            /*
            var details = dal.GetCompanyInfo();
            var compConfig = dal.GetConfiguration();

            details.CompanyName = compConfig.CompanyName;
            details.LogoPath1 = compConfig.LogoPath1;
            details.LogoPath2 = compConfig.LogoPath2;
            details.LogoPath3 = compConfig.LogoPath3;
            details.LogoPath4 = compConfig.LogoPath4;
            details.OsekNo = compConfig.OsekNo;
            */
